...
public class HttpRequestParameterAdapter {   // getParameter()하지 않고 한 번에 처리한다…
	...
	public static <T>T execute(HttpServletRequest request, Class<T> className) throws Exception{
		...
		for(Method method : methods) {
			if(method.getName().indexOf("set") == 0) { //세터이면
				...
				String[] paramValues = request.getParameterValues(requestParamName); //파라미터 이름으로 모든 파라미터의 벨류 추출 -> 스트링 배열로
